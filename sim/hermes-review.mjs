/** Photograph the actual excavation from above, at an angle, and close to its face. */
import { createServer } from 'node:http';
import { readFile, mkdir, writeFile } from 'node:fs/promises';
import { resolve, extname, join } from 'node:path';
import { chromium } from 'playwright';
import { HERMES, hermesWorld } from '../src/app/hermes.js';
import { skyAt } from './sky.mjs';
import { PLACE } from './place.mjs';
const root=resolve('dist'),out=resolve(process.argv[2]||'.local/hermes/review');
...
try{
 ...
 const shots=[{name:'overhead',distance:29,x:0,z:0,polar:.08,az:0},{name:'excavation',distance:27,x:0,z:-1,polar:.80,az:.65},{name:'face',distance:8,x:-.2,z:-7.8,polar:.6,az:0}];
 for(const v of shots){
  const target=hermesWorld(v.x,v.z);
  ...
  },{v,target,rot:HERMES.rot});
  const deadline=Date.now()+90000;let settled=0;
  while(settled<15){
   const state=await page.evaluate(()=>{const w=window.__world;w.step(1);const h=w.digs.group.children.find(x=>x.name==='Hermes — The Buried Messenger');return{ready:w.startup.complete,scan:h?.userData.ready,pending:w.terrain.pendingVisible};});
   settled=state.ready&&state.scan&&state.pending===0?settled+1:0;
   if(Date.now()>deadline)throw new Error('Excavation did not load: '+JSON.stringify({state,errors}));
   await new Promise(r=>setTimeout(r,10));
  }
  await page.screenshot({path:join(out,v.name+'.png'),animations:'disabled'});
 }
 ...
 console.log('Hermes review passed',JSON.stringify(stats));
