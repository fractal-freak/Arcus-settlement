...
test('proposal continues a retained draft and adds new modules against original preimages',async()=>{
 ...
 try{
  ...
  for(const f of ['before.png','before-stone.png','before-gatehouse.png'])await writeFile(join(root,'.local/api-development',f),'mock image');
  await writeFile(join(root,'fake-api.mjs'),`globalThis.fetch=async(url,init)=>{const text=JSON.parse(init.body).messages[0].content[0].text;if(!text.includes('continue useful foundation'))throw Error('Missing durable feedback');const value=text.includes('Respond {"paths"')?{paths:['src/app/example.js']}:{summary:'extend draft',files:[{path:'src/app/example.js',content:'continued'},{path:'sim/systems/bakery.mjs',content:'export const bakery = true;'}]};return {ok:true,json:async()=>({choices:[{message:{content:JSON.stringify(value)}}]})};};`);
